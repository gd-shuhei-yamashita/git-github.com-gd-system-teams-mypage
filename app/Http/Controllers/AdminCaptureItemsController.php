 ...
 * 請求データ取込
 */
class AdminCaptureItemsController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        return view('admin/capture_items');
    }
    ...
    public function Registration(Request $request)
    {
      ...
      for($r = 1; $r <= $rowMax; $r ++) {
        ...
        if (mb_strlen($sheetData[$r][1] , '8bit') != 19 ) {
          $tempv = $sheetData[$r];
          $tempv[1] = "<div class='red'>" . $tempv[1] . "</div>";
          array_unshift($tempv , $r);
          array_push($tempv, "1列目が請求番号(19桁)ではありません");
          $resNg ++;
          // 先頭RETURN_MAX行返す
        ...
        if (mb_strlen($sheetData[$r][2] , '8bit') != 55 && mb_strlen($sheetData[$r][2] , '8bit') != 50 && mb_strlen($sheetData[$r][2] , '8bit') != 43) {
          $tempv = $sheetData[$r];
          $tempv[2] = "<div class='red'>" . $tempv[2] . "</div>";
          array_unshift($tempv , $r);
          array_push($tempv, "2列目が内訳コード(43桁,50桁または55桁)ではありません");
          $resNg ++;
          // 先頭RETURN_MAX行返す
          if ($resNg <= self::RETURN_MAX) {
            $errorList[] = $tempv;
        ...
        if (!preg_match('/^(\d+)$/',$sheetData[$r][3])) {
          ...
          array_unshift($tempv , $r);
          array_push($tempv, "3列目が明細表示順(数字)ではありません");
          $resNg ++;
          // 先頭RETURN_MAX行返す
          if ($resNg <= self::RETURN_MAX) {
            $errorList[] = $tempv;
        ...
        if (mb_strlen($sheetData[$r][4] , '8bit') == 0) {
          $tempv = $sheetData[$r];
          $tempv[4] = "<i class='material-icons'>error</i><div class='red'>" . $tempv[4] . "</div>";
          array_unshift($tempv , $r);
          array_push($tempv, "4列目に内訳名の記載がありません");
          $resNg ++;
          // 先頭RETURN_MAX行返す
          if ($resNg <= self::RETURN_MAX) {
            $errorList[] = $tempv;
        ...
        if (!preg_match('/^([0-9.\-]+)$/',$sheetData[$r][5])) {
          $tempv = $sheetData[$r];
          $tempv[5] = "<div class='red'>" . $tempv[5] . "</div>";
          array_unshift($tempv , $r);
          array_push($tempv, "5列目が内訳金額(数値)ではありません");
          $resNg ++;
          // 先頭RETURN_MAX行返す
          if ($resNg <= self::RETURN_MAX) {
            $errorList[] = $tempv;
      ...
      try
      {
        ...
        foreach ($sheetData as $temp_data) {
          ...
          $BillingItemize = BillingItemize::firstOrNew([
            "billing_code" => $temp_data[1],
            "itemize_code" => $temp_data[2],
            "itemize_order" => $temp_data[3],
            //(int)str_replace("/","",$temp_data[2]),
          ], [
            // "itemize_name" => $temp_data[4],
            // "itemize_bill" => $temp_data[5],
            // "note"         => $temp_data[6],
          ...
          if ($BillingItemize->exists) {
            // Update
            $BillingItemize->itemize_name = $temp_data[4];
            $BillingItemize->itemize_bill = $temp_data[5];
            $BillingItemize->note = $temp_data[6];

            $BillingItemize->updated_user_id = $user_code;
            $resUpd ++;
            ...
            $BillingItemize->created_user_id = $user_code;
            $BillingItemize->updated_user_id = $user_code;
            $resIns ++;
          }
          $BillingItemize->save();
