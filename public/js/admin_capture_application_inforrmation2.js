...
function loader1_refresh() {
  ...
  for(let i in read_results.sheet_data) {
    ...
    $('table#result1_list tbody').append(
      ...
      +'<td>'+ temp_linedata[9] +'</td>' // エラーメッセージ等
      +'</tr>'
      );
  }
...
$('#submit_section').submit(function(event) {
  ...
  if (!post_data['btn_upload_section']) {
    error_message = '契約データ の csvファイルを登録して下さい';
  ...
  // postする
  $.ajax({
    url: application_url + "admin/capture_application_information2",
    type: 'post',
    dataType: 'json',
    data : post_data,
    ...
    if (response.status == 200) {
      ...
      // M.toast({html: '契約データ 登録完了しました'});
      alert('契約データ 登録完了しました');
