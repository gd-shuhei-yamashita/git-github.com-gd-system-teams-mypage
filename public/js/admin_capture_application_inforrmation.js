...
function loader1_refresh() {
  ...
  for(let i in read_results.sheet_data) {
    ...
    $('table#result1_list tbody').append(
      ...
      +'<td>'+ temp_linedata[7] +'</td>' // エラーメッセージ等
      +'</tr>'
      );
  }
...
$('#submit_section').submit(function(event) {
  ...
  if (!post_data['btn_upload_section']) {
    error_message = '顧客データ の csvファイルを登録して下さい';
  ...
  // postする
  $.ajax({
    url: application_url + "admin/capture_application_information",
    type: 'post',
    dataType: 'json',
    data : post_data,
    ...
    if (response.status == 200) {
      ...
      // M.toast({html: '顧客データ 登録完了しました'});
      alert('顧客データ 登録完了しました');
