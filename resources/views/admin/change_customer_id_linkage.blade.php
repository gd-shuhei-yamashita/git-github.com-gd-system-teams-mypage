...
@section('content')
        ...
        <div class="l-main">
            ...
            <div class="input-field-text">
                <a>
                    契約データ（Contract）、請求データ（Billing）、使用率（UsageT）テーブルの、マイページ ID を一斉に書き換えます。<br class="spno">
                    更新履歴を画面で確認可能です。管理画面より直接もとに戻せる手順ではないため、特に作業にご注意ください。<br class="spno">
                    一時作業領域として、マイページIDにADMN000001が利用可能です。<br>
                    <span>（pptに予定されていた文言では）マイページIDに紐付く供給地点特定番号を変更する</span>
                </a>
            </div>
            <div class="mds">
            <p>新規登録</p>
                    ...
                    <div class="form-area info">
                        <form id="submit_section" action="{{ route('change_customer_id_linkage_store') }}" method="post">
						<input type="hidden" name="_token" id="csrf-token" value="{{ Session::token() }}" />
                            <div class="input-area">
                                <div class="input-field input-field1">
                                    <label>(旧)マイページID</label>
                                    <input name="now_customer_code" id="now_customer_code" type="text" class="validate" value="{{old('now_customer_code')}}" placeholder="例:MC00000000" >
									<span class="help-block red-text">{{$errors->first('now_customer_code')}}</span>
                                </div>
                                <div class="input-field">
                                    <label>(新)マイページID</label>
                                    <input placeholder="例:MC00000000" name="new_customer_code" id="new_customer_code" type="text" class="validate" value="{{old('new_customer_code')}}">
									<span class="help-block red-text">{{$errors->first('new_customer_code')}}</span>
                                </div>
                            </div>
...
<div class="container">
  <div class="col s12">
  <!-- マイページID紐付変更 -- >
    <div class="section">契約データ(Contract)、請求データ(Billing)、使用率(UsageT)テーブルの、マイページIDを一斉に書き換えます<br/>
    更新履歴を画面で確認可能です。管理画面より直接もとに戻せる手順ではないため、特に作業にご注意ください。<br/>
    一時作業領域として、マイページIDに ADMN000001 が利用可能です。<br/>
    <span class="grey-text">(pptに予定されていた文言では)マイページIDに紐付く供給地点特定番号を変更する</span></div>
      ...
      <form id="submit_section" action="{{ route('change_customer_id_linkage_store') }}" method="post">
        ...
        <div class="row">
          ...
          <i class="material-icons prefix">account_circle</i>
            <input name="now_customer_code" id="now_customer_code" type="text" class="validate" value="{{old('now_customer_code')}}" placeholder="例:MC00000000" >
            <label for="now_customer_code">マイページID(旧)</label>
            <span class="help-block red-text">{{$errors->first('now_customer_code')}}</span>
          </div>
        </div>
        <div class="row">
          ...
          <i class="material-icons prefix">account_circle</i>
            <input placeholder="例:MC00000000" name="new_customer_code" id="new_customer_code" type="text" class="validate" value="{{old('new_customer_code')}}">
            <label for="new_customer_code">マイページID(新)</label>
            <span class="help-block red-text">{{$errors->first('new_customer_code')}}</span>
          </div>
        </div>
...
@section('pageJs')
<script src="{{asset('js/admin_change_customer_id.js') }}"></script>
