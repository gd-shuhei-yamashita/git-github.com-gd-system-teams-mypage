...
<div class="container">
  ...
  <!-- 使用量データ取込 -->
    ...
    <div class="row content header">
      ...
      <form method="POST" action="{{ route('store_usagedata')}}" enctype="multipart/form-data" style="display:inline-block;">
        <input type="hidden" name="_token" id="csrf-token" value="{{ Session::token() }}" />
        <input type="file" id="file" name="file[]" class="form-control" multiple required>
        <button type="submit">アップロード</button>
      </form>

      <button id="execute-usage" onclick="location.href='/admin/batch/usagedata'">取込実行</button>
