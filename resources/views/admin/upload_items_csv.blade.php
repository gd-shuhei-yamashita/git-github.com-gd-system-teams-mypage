...
<div class="container">
  ...
  <!-- 内訳データ取込 -->
    ...
    <div class="row content header">
      ...
      <form method="POST" action="{{ route('store_meisaidata')}}" enctype="multipart/form-data" style="display:inline-block;">
        <input type="hidden" name="_token" id="csrf-token" value="{{ Session::token() }}" />
        <input type="file" id="file" name="file[]" class="form-control" multiple required>
        <button type="submit">アップロード</button>
      </form>

      <button id="execute-meisai" onclick="location.href='/admin/batch/meisaidata'">取込実行</button>
